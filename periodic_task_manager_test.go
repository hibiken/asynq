...
func TestNewPeriodicTaskManager(t *testing.T) {
	cfgs := []*PeriodicTaskConfig{
		{Cronspec: "* * * * *", Task: NewTask("foo", nil, nil)},
		{Cronspec: "* * * * *", Task: NewTask("bar", nil, nil)},
	}
	tests := []struct {
		desc string
...
func TestPeriodicTaskConfigHash(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			a: &PeriodicTaskConfig{
				Cronspec: "* * * * *",
				Task:     NewTask("foo", []byte("hello world!"), nil),
				Opts:     []Option{Queue("myqueue")},
			},
			b: &PeriodicTaskConfig{
				Cronspec: "* * * * *",
		...
		},
		{
			...
			a: &PeriodicTaskConfig{
				Cronspec: "* * * * *",
				Task:     NewTask("foo", []byte("hello!"), nil),
				Opts:     []Option{Queue("myqueue")},
			},
			b: &PeriodicTaskConfig{
				Cronspec: "* * * * *",
...
func TestPeriodicTaskManager(t *testing.T) {
	...
	cfgs := []*PeriodicTaskConfig{
		{Task: NewTask("task1", nil, nil), Cronspec: "* * * * 1"},
		{Task: NewTask("task2", nil, nil), Cronspec: "* * * * 2"},
	}
	...
	provider.SetConfigs([]*PeriodicTaskConfig{
		{Task: NewTask("task1", nil, nil), Cronspec: "* * * * 1"},
		{Task: NewTask("task3", nil, nil), Cronspec: "* * * * 3"},
	})
