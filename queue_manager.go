...
// GetSetOfQueues returns unordered set of queues from the broker.
...
func (qm *queueManager) updateDynamicQueues() {
	...
	queues, err := qm.broker.SetOfQueues(context.Background())
	if err != nil {
	...
	for queue := range queues {
		...
		priority := qm.matcher.GetPriority(queue)
...
//	                           // if default priority is not set, asynq will use the default priority of 0 for unknown queues. See: internal/base/DefaultQueuePriority
//	},
...
func (d *queuePriorityMatcher) GetPriority(queueName string) int {
	if priority, ok := d.exact[queueName]; ok {
		// return priority for exact match
		return priority
	}
	// find the longest match in wildcard patterns
	longestMatchLen, matchedPriority := -1, base.DefaultQueuePriority
